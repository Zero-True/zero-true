...
def test_execute_request_simple_variable():
    ...
    req = Request(originId="0", cells=[CodeRequest(id="0", code="a = 1", variable_name="", nonReactive=False, cellType='code')], components={})
...
def test_execute_request_function_definition():
    notebook_state = setup_function()
    req = Request(originId="1", cells=[CodeRequest(id="1", code="def add(x, y): return x + y", variable_name="", nonReactive=False, cellType='code')], components={})
    execute_thread = threading.Thread(target=execute_request, args=(req, notebook_state))
    execute_thread.start()
    execute_thread.join()
...
def test_execute_request_import_module():
    notebook_state = setup_function()
    req = Request(originId="2", cells=[CodeRequest(id="2", code="import math", variable_name="", nonReactive=False, cellType='code')], components={})
    execute_thread = threading.Thread(target=execute_request, args=(req, notebook_state))
    execute_thread.start()
    execute_thread.join()
...
def test_execute_request_multiple_cells_with_dependencies():
    notebook_state = setup_function()
    req = Request(originId="3", cells=[CodeRequest(id="3", code="a = 1", variable_name="", nonReactive=False, cellType='code'),
                        CodeRequest(id="4", code="b = a + 1", variable_name="", nonReactive=False, cellType='code')], components={})
    execute_thread = threading.Thread(target=execute_request, args=(req, notebook_state))
    execute_thread.start()
    execute_thread.join()
