...
def get_parent_vars(
    ...
    if cell_id not in code_components.cells:
        ...
        raise KeyError(
            f"'{cell_id}' is not present in code_components. Execution stopped."
        )
    ...
    for parent_cell in parent_cells:
        ...
        parent_vars_list = (
            code_components.cells[parent_cell].loaded_names
    ...
    with UserContext(state) as execution_state:
        ...
        if request.originId:
            if request.reactiveMode:
                ...
                try:
                    ...
                    no_longer_dependent_cells = set(old_downstream_cells) - set(
                        downstream_cells
                    )

                    if no_longer_dependent_cells:
                        downstream_cells.extend(
                            list(OrderedDict.fromkeys(no_longer_dependent_cells))
                        )
                except Exception as e:
                    logger.error(
                        "Error while updating cell dependencies: %s",
            ...
            downstream_cells = [
                cell.id for cell in request.cells if cell.cellType in ["code", "sql"]
            ]
        for code_cell_id in downstream_cells:
            ...
            try:
                ...
                logger.debug(
                    "Error while getting cell layout, setting empty layout: %s",
                    traceback.format_exc(),
                )
                layout = Layout(**{})
...
def execute_cell(
    ...
    with contextlib.redirect_stdout(WebSocketStream()), contextlib.redirect_stderr(
        WebSocketStream()
    ):
        try:
    ...
    execution_state.cell_outputs_dict[code_cell_id] = {
        k: try_pickle(v) for k, v in temp_globals.items() if k != "__builtins__"
    }
